...
class LatentArrayTransformer(nn.Module):
    ...
    # self.model((c_in * x).to(dtype), c_noise.flatten(), cond=cond_emb, **model_kwargs)
    def forward(self, x, t, cond=None):
        t_emb = self.map_noise(t)[:, None]
        t_emb = F.silu(self.map_layer0(t_emb))
        t_emb = F.silu(self.map_layer1(t_emb))

        x = self.proj_in(x) # [B, M, C0] ([B, 512, 32]) -> [B, M, head*dim]([B, 512, 512]) 8头维度64
...
class EDMLoss:
    ...
    def __call__(self, net, inputs, labels=None, augment_pipe=None):
        # net: 条件diffusion model
        # inputs: VAE latent feature [B, M, C0]
        # labels: 物体的category
        rnd_normal = torch.randn([inputs.shape[0], 1, 1], device=inputs.device) # [B, 1, 1]
        # rnd_normal = torch.randn([1, 1, 1], device=inputs.device).repeat(inputs.shape[0], 1, 1)

        sigma = (rnd_normal * self.P_std + self.P_mean).exp() # 噪声水平
...
class EDMPrecond(torch.nn.Module):
    def __init__(self,
        ...
        # 未提供context dim, 则context_dim = query_dim = 512
        self.model = LatentArrayTransformer(in_channels=channels, t_channels=256, n_heads=n_heads, d_head=d_head, depth=depth, context_dim=1536)

        self.category_emb = nn.Embedding(55, n_heads * d_head) # [55] -> [8 * 64]

        # self.CLIPImageEncoder = CLIPModel.from_pretrained("openai/clip-vit-base-patch32")
        # for param in self.CLIPImageEncoder.parameters():
        #     param.requires_grad = False
        # self.CLIPImageProcessor = CLIPImageProcessor.from_pretrained("openai/clip-vit-base-patch32")
    ...
    def forward(self, x, sigma, class_labels=None, force_fp32=False, **model_kwargs):
        ...
        # else:
        #     # category_emb表示一个词表，总索引数为55(一共有55个类)，每个索引代表长度为n_heads * d_head = 8 * 64 = 512维度的向量
        #     # 经过查询, [B, 1]的class_labels会变成[B, 512], 然后unsqueeze dim=1, 变成[B, 1, 512]
        ...
        # inputs = self.CLIPImageProcessor(images=class_labels, return_tensors="pt", do_rescale=False)
        # inputs['pixel_values'] = inputs['pixel_values'].to(x.device, non_blocking=True)
        # condition = self.CLIPImageEncoder.get_image_features(**inputs).unsqueeze(1)

        condition = class_labels

        # conditon = conditon.unsqueeze(1)
        # [B, 512] -> [B, 1, 512]

        F_x = self.model((c_in * x).to(dtype), c_noise.flatten(), cond=condition, **model_kwargs)
        assert F_x.dtype == dtype
        D_x = c_skip * x + c_out * F_x.to(torch.float32)
        return D_x # Denoiser 输出
...
def kl_d512_m512_l32_d24_edm():
    ...
    return model
    for dialog in sample_dialogue:
        name = dialog["role"]
        print(f"name = {name}")
        if name == "persona":
            dialog["role"] = persona_name
        elif name == "character":
            dialog["role"] = character_name

    print(f"model = {model}")
    if model == "evomuse":
        system_str = make_evomuse_system_str(character_name, persona_name)
    else:
        system_str = ""
