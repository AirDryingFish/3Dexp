...
class AutoEncoder(nn.Module):
    ...
    def encode(self, pc):
        ...
        # exam:
        #   B = 4, N = 3
        #   batch = [0, 0, 0, 1, 1, 1, 2, 2, 2, 3, 3, 3]
        batch = torch.arange(B).to(pc.device)
        batch = torch.repeat_interleave(batch, N)

        pos = flattened

        # 采样 (512 / 2048) = 25% 的点，即最终点的维度是 512 (M)
        ...
        sampled_pc = pos[idx]
        sampled_pc = sampled_pc.view(B, -1, 3) # B, M, 3
        ######

        sampled_pc_embeddings = self.point_embed(sampled_pc) # [B, M, 3] -> [B, M, C]

        pc_embeddings = self.point_embed(pc) # [B, N, 3] -> [B, N, C]
    ...
    def decode(self, x, queries):
        ...
        #           get_latent_ff(**cache_args)
        #       ]))
        #   其中depth为24。创建24层的self attention层
        ...
        # 经过self attention之后的x，与pos embedding之后的queries算cross-attention
        # [B, N, C] AND [B, M, C] -> [B, N, C]
        ...
        # 最后通过FC层，输出一个一维的预测occ
        # [B, N, C] -> [B, N, 1]
        return self.to_outputs(latents)

    def forward(self, pc, queries):
        # outputs = model(surface -> pc, points -> queries)
        x = self.encode(pc) # [B, M, C]

        o = self.decode(x, queries).squeeze(-1) # [B, N, 1] -> [B, N]
